...
import { secondsElapsed } from './helpers';
...
export const getTimeUntilEvent = (
  ...
  /** We will use this only if the event time crosses midnight UTC */
  let endTime;

  // console.log(
  //   '[GET]',
  //   e.start.toFormat('HH:mm:ss'),
  //   e.end.toFormat('HH:mm:ss'),
  ...
  //       timeUntil = secondsElapsed(
  //         now,
  //         DateTime.fromMillis(e.end.toMillis()).plus({ day: 1 })
  //       );
  //     }
  //   } else {
  //     /**
  ...
  //       timeUntil = secondsElapsed(
  //         now,
  //         DateTime.fromMillis(e.start.toMillis()).plus({ day: 1 })
  //       );
  ...
  if (e.start > e.end) {
    // IF start is AFTER end, add an hour to the end time for calcs
    endTime = DateTime.fromMillis(e.end.toMillis()).plus({ day: 1 });
    // The phase is active IF
  }

  const active = now >= e.start && now < (endTime || e.end);
  const timeUntil = secondsElapsed(now, active ? endTime || e.end : e.start);

  // console.log('[GET RES]', event, active, timeUntil);
